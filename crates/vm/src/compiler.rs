...
impl Compiler {
    ...
    pub fn new() -> Self {
        ...
        let builtins = [
            "print", "int", "float", "str", "len", "range", 
            "list", "reversed", "input", "open"
        ];
    ...
    pub fn compile_program(mut self, heap: &mut crate::heap::Heap, mir_program: &MIRProgram) -> Result<Chunk, VyprError> {
        ...
        for func in &mir_program.functions {
            ...
            let func_chunk = func_compiler.compile_function(heap, func)?;

            let func_val = Value::allocate_function(
                heap,
                func.arity, 
                func.locals.len(), 
                Rc::new(func_chunk)
    ...
    pub fn compile_function(mut self, heap: &mut crate::heap::Heap, mir_func: &MIRFunction) -> Result<Chunk, VyprError> {
        for (bb_idx, block) in mir_func.basic_blocks.iter().enumerate() {
            ...
            for stmt in &block.statements {
                self.compile_statement(heap, stmt, &mir_func.locals)?;
            }

            self.compile_terminator(heap, &block.terminator, current_bb, Span::default())?;
        }
    ...
    fn compile_statement(&mut self, heap: &mut crate::heap::Heap, stmt: &Statement, _locals: &[LocalDecl]) -> Result<(), VyprError> {
        let span = stmt.span;

        match &stmt.kind {
            StatementKind::Assign(place, rval) => {
                self.compile_rvalue(heap, rval, span)?;
                    ...
                    for (i, proj) in place.projection.iter().enumerate() {
                        ...
                        match proj {
                            ...
                            ProjectionElem::Property(name) => {
                                let name_idx = self.chunk.add_constant(Value::make_string(heap, name));
                                if is_last {
            ...
            StatementKind::AssertType(operand, ty) => {
                ...
                if data_type != DataType::Any {
                    self.compile_operand(heap, operand, span)?;
    ...
    fn compile_rvalue(&mut self, heap: &mut crate::heap::Heap, rval: &Rvalue, span: Span) -> Result<(), VyprError> {
        match rval {
            Rvalue::Use(operand) => self.compile_operand(heap, operand, span)?,
            
            Rvalue::BinaryOp(op, lhs, rhs) => {
                self.compile_operand(heap, lhs, span)?;
                self.compile_operand(heap, rhs, span)?;
            ...
            Rvalue::ListInit(elements) => {
                for element in elements {
                    self.compile_operand(heap, element, span)?;
                }
            ...
            Rvalue::DictInit(keys, values) => {
                for (k, v) in keys.iter().zip(values.iter()) {
                    self.compile_operand(heap, k, span)?;
                    self.compile_operand(heap, v, span)?;
                }
                self.chunk.write(OpCode::BuildDict(keys.len()), span);
            }

            Rvalue::Import(module) => {
                let name_idx = self.chunk.add_constant(Value::make_string(heap, module));
            ...
            Rvalue::FunctionDef(function) => {
                ...
                let chunk = compiler.compile_function(heap, function)?;

                let val = Value::allocate_function(
                    heap,
                    function.arity,
                    function.locals.len(),
                    Rc::new(chunk),
            ...
            Rvalue::FormatString(ops) => {
                for op in ops {
                    self.compile_operand(heap, op, span)?;
                }
            ...
            Rvalue::ListAppend(list_op, item_op) => {
                self.compile_operand(heap, list_op, span)?;
                self.compile_operand(heap, item_op, span)?;
    ...
    fn compile_operand(&mut self, heap: &mut crate::heap::Heap, operand: &Operand, span: Span) -> Result<(), VyprError> {
        match operand {
            Operand::Copy(place) => {
                self.chunk.write(OpCode::GetLocal(place.local.0), span);
            ...
            Operand::Const(c) => {
                let val = match c {
                    vir::context::Constant::Int(i) => Value::int(*i as i32),
                    vir::context::Constant::Float(f) => Value::float(*f),
                    vir::context::Constant::String(s) => Value::make_string(heap, s),
                    vir::context::Constant::Bool(b) => Value::boolean(*b),
    ...
    fn compile_terminator(&mut self, heap: &mut crate::heap::Heap, terminator: &Terminator, current_bb: BasicBlockID, span: Span) -> Result<(), VyprError> {
        match terminator {
            Terminator::Goto { target } => {
                self.emit_jump_or_loop(*target, current_bb, OpCode::Jump, span);
            }
            Terminator::SwitchInt { discriminant, true_target, false_target } => {
                self.compile_operand(heap, discriminant, span)?;
            ...
            Terminator::Call { callee, args, kwargs, destination, target } => {
                self.compile_operand(heap, callee, span)?;

                for arg in args {
                    self.compile_operand(heap, arg, span)?;
                }
            ...
            Terminator::MethodCall { object, method_name, args, kwargs, destination, target } => {
                self.compile_operand(heap, object, span)?;
                ...
                for (name, arg) in kwargs {
                    let kw_name_idx = self.chunk.add_constant(Value::make_string(heap, name));
                    self.chunk.write(OpCode::Constant(kw_name_idx), span);
                    self.compile_operand(heap, arg, span)?;
                }

                let name_idx = self.chunk.add_constant(Value::make_string(heap, method_name));
